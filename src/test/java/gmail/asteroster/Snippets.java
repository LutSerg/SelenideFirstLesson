...
public class Snippets {
    void browser_command_examples() {
        ...
        //используется в экранаъ авторизации
        ...
        Selenide.clearBrowserLocalStorage(); //очистка LocalStorage
    ...
    void selectors_examples() {
        ...
        $(byText("full text")).click(); //ищет по полному тексту
        $(withText("ull tex")).click(); //ищет по подтексту

        $(byTagAndText("div","full text"));
        $(withTagAndText("div","ull text"));

        $("").parent(); //родительский элемент на 1 уровень вверх
        $("").sibling(1); //брат/сестра вниз
        $("").preceding(1); //брат/сестра вверх
        $("").closest("div");
    ...
    void actions_examples() {
        $("").click();
        $("").doubleClick();
        $("").contextClick(); //нажатие правой кнопкой мыши

        $("").hover();

        $("").setValue("text"); //очищает поле и заполняет вводимым значением
        $("").append("text"); //не очщиает поле и дописывает значение к существующему
        $("").clear();
        $("").setValue(""); // clear
    ...
    void conditions_examples() {
        ...
        $("").shouldHave(text("abc")); //может искать неполное совпадение, например, если буквы входят в более длинное слово
        $("").shouldHave(exactText("abc")); //ищет точное совпадение
        $("").shouldHave(textCaseSensitive("abc")); //с регистром
        $("").shouldHave(exactTextCaseSensitive("abc")); //точное совпадение с регистром
        $("").should(matchText("[0-9]abc$"));

        $("").shouldHave(cssClass("red")); //проверка знаения класса
        $("").shouldHave(cssValue("font-size", "12")); //физ.свойство элемента (цвет, размер ит.д.)

        $("").shouldHave(value("25")); //значение тектсовой строки?
        $("").shouldHave(exactValue("25"));
        $("").shouldBe(empty);

        $("").shouldHave(attribute("disabled")); //наличие атрибута
        $("").shouldHave(attribute("name", "example")); //значение атрибута
